...
# st.title("🌱 AgriSense: Crop Disease & Fertilizer Recommendation System")
...
# powdery_white = st.checkbox("Powdery White Coating")
# black_sooty = st.checkbox("Black Sooty Mold")

# st.header("Pest Presence")
# aphids = st.checkbox("Aphids")
# mites = st.checkbox("Mites")
...
#     # Declare all facts from user input
...
#     if diagnoses:
#         for d in diagnoses:
#             st.json({k: v for k, v in d.items() if k != "__factid__"})
#     else:
...
#     if recs:
#         for r in recs:
#             st.json({k: v for k, v in r.items() if k != "__factid__"})
#     else:
